...
async function identifyAtoms(sectionData: any, sectionName: string, claudeApiKey: string): Promise<Array<{name: string, type: string, id: string}>> {
  ...
  // Filter out icons, vectors, and logos before analysis
  ...
  console.log(`      🎯 Filtered to ${filteredElements.length} UI components (excluded ${allElements.length - filteredElements.length} icons/vectors)`)

  // Debug: show first few filtered elements
  if (filteredElements.length > 0) {
    console.log(`      🔍 Sample UI components:`)
    filteredElements.slice(0, 3).forEach(el => {
  ...
  try {
    ...
    // Filter out icons/vectors and only return UI components (using filtered elements)
    return filteredElements.filter(el =>
      ...
      el.name.toLowerCase().includes('navigation')
    ).map(el => ({
      id: el.id,
      name: el.name,
      type: el.type === 'TEXT' ? 'text' : 'button'
    }))
  }
}
